"""
车辆轨迹的处理类，包括从轨迹中计算车道方向，车辆轨迹的保存和加载，并提供展示轨迹相关展示接口
"""
...
from utils.draw_utils import draw_speed_cluster
...
class TraceProcessor:
    def __init__(self, trace_dict=None, csv_file_path=None):
        if csv_file_path is not None:
            self.load_trace_from_csv(csv_file_path)
        else:
            self.trace_dict = trace_dict

    def cal_lane_from_trace(self, start_frame, time_span=10):
        """
        ...
        lane_direction = {}
        # 获取指定帧的所有车辆速度矢量
        speed_vector_ls = self.__extract_speed_vector_from_trace(start_frame, time_span)
        if len(speed_vector_ls) == 0:
            return lane_direction
        # 对车辆速度矢量进行聚类
        cluster_dict = self.__cluster_speed_vector(speed_vector_ls)
        # 使用统计方法剔除明显偏离车道方向的速度矢量
        self.__filterSpeed(cluster_dict)
    ...
    def show_speed_cluster(self):
        """
        查看车辆速度矢量的聚类情况
        """
        speed_ls = self.__extract_speed_vector_from_trace(100)
        cluster_dict = self.__cluster_speed_vector(speed_ls)

        draw_speed_cluster(cluster_dict)

    def __extract_speed_vector_from_trace(self, extract_frame, time_span=10):
        """
        ...
        """
        speed_vector_ls = []
        for obj_id, trace_ls in self.trace_dict.items():
    ...
    def load_trace_from_csv(self, csv_path):
        """
        从csv文件中加载轨迹
        """
        self.trace_dict = {}
    ...
    def extract_trace_to_csv(self, output):
        """
        从视频中提取轨迹并保存到csv文件
    ...
    @classmethod
    def norm_speed_vector(cls, speed_ls):
        """
    ...
    @classmethod
    def __cluster_speed_vector(cls, speed_vector_ls):
        """
        对车辆的速度矢量进行聚类，返回聚类结果
        """
    ...
    @classmethod
    def __filterSpeed(cls, cluster_dict):
        """
        使用统计方法剔除明显偏离车道方向的速度矢量
        """
        for cluster_id, speed_vector_ls in cluster_dict.items():
            # 对速度矢量归一化来描述速度方向
            angles = cls.norm_speed_vector(speed_vector_ls)
